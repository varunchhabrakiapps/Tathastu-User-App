...
/**
 * Manual location search — ghost glass field that filters supported metros as you type.
 */
export const LocationSearchBar = memo(function LocationSearchBar({
  value,
  onChangeText,
  onSubmitEditing,
}: Props) {
    ...
    >
      <LiquidGlassMaterial preset="ghost" borderRadius={radii.full} className="overflow-hidden rounded-full">
        <View className="flex-row items-center gap-3 px-4 py-3.5">
          ...
          />
          <TextInput
            accessibilityLabel={t('screens.manualLocation.search.accessibilityLabel')}
            accessibilityHint={t('screens.manualLocation.search.accessibilityHint')}
            value={value}
            onChangeText={onChangeText}
            onSubmitEditing={onSubmitEditing}
            placeholder={t('screens.manualLocation.search.placeholder')}
          ...
          {hasValue ? (
            <Pressable
              ...
              accessibilityLabel={t('screens.manualLocation.search.clearA11y')}
              hitSlop={10}
